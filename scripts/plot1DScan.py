...
def read(scan, param, files, ycut, scale_poi=1.0):
...
def BuildScan(scan, param, files, color, yvals, ycut, scale_poi=1.0):
    graph = read(scan, param, files, ycut, scale_poi)
    if graph.GetN() <= 1:
        graph.Print()
...
parser.add_argument("--scale-POI", type=float, default=1.0, help="Scale the POI value by this factor")
...
main_scan = BuildScan(args.output, args.POI, [args.main], args.main_color, yvals, args.y_cut, args.scale_POI)
...
if args.others is not None:
    for oargs in args.others:
        ...
        other_scans.append(BuildScan(args.output, args.POI, [splitargs[0]], int(splitargs[2]), yvals, args.y_cut, args.scale_POI))
